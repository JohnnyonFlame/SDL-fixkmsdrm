        ...
        if((fdDrmMode = fopen("/var/run/drmMode", "r")) != NULL) {
            if(fscanf(fdDrmMode, "%i", &drmMode) == 1) {
                if(drmMode>=0 && drmMode<connector->count_modes) {
                    ...
                    if(pcrtc != NULL)
                        KMSDRM_drmModeSetCrtc(viddata->drm_fd, pcrtc->crtc_id,
                            pcrtc->buffer_id, 0, 0, &connector->connector_id, 1,
                            &connector->modes[drmMode]);
                }
            }
...
void KMSDRM_GetDisplayModes(_THIS, SDL_VideoDisplay *display)
{
    ...
    for (i = 0; i < conn->count_modes; i++) {
        ...
        if((wantedMode >= 0) && (i != wantedMode)) continue; // batocera
